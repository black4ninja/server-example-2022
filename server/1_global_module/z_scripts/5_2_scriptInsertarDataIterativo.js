...
function iterateFiles(index,files){
    ...
    if(index >= files.length){
			...
			.on('data',(buffer)=>{
		    //log(JSON.parse(buffer.toString("utf8"))["nombre_act"])
		    //log(JSON.parse(buffer.toString("utf8")))
			...
			.on('done',(error)=>{
			    ...
			    var reduccion = 0
			    array_data = array_data.splice(reduccion);
			    log(array_data.length)
			    newIndex = reduccion
			   	manipulateData()
					//iterateFiles((index+1), files)
			    /*manipulateChunkData(106893).then(function(){
				    //resolve()
				    iterateFiles((index+1), files)
				  })*/ 
...
async function manipulateData()
{
  ...
  for (var z = indexItems; z < array_data.length; z++) {
  	const element = array_data[z]
  	searchIndex.push(element["id"])
		subArray.push(element)
    if(subArray.length == 500 || indexItems == array_data.length -1){
    	const start = now()
    	log(indexItems + " - " + (indexItems + 1) + " - " + moment().format("HH:mm"))
    	log("Nuevo Indice -" + (newIndex + (indexItems+1)))
    	//log("Paquete subida")
    	var Parse = require('parse/node');
	   ...
	   await Parse.Object.saveAll(updatableArray)
	   await Parse.Object.saveAll(newArray)
		results = []
		subArray = []
		searchIndex = []
		newArray = []
