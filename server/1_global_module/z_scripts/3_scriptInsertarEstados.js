	...
	return new Promise((resolve,reject)=>{
		...
		if(index >= json.length){
			...
			while(subIndex < 999 && index < json.length){
				searchIndex.push(json[index]["entidad"])
				subArray.push(json[index]["entidad"])
				index++
				subIndex++
			}
		    ...
		    for(var i = 0; i < subArray.length; i++){
		    	...
		    	if(!found){
		    		const Table = Parse.Object.extend("DENUEEstados");
...
async function queryTable(searchIndex) {
  ...
  var Table = Parse.Object.extend("DENUEEstados");
